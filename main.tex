...
\begin{equation}
w_v = \frac{q^{(v)}}{\sum_{u=1}^{V} q^{(u)}}, \quad q^{(v)} = \frac{\mu_1}{1 + \text{CV}(\hat{p}^{(v)})} + \frac{\mu_2}{1 + \bar{d}^{(v)}_{\text{avg}}}
\end{equation}
where $\text{CV}(\cdot)$ is the coefficient of variation and $\bar{d}^{(v)}_{\text{avg}}$ is the average neighbor distance.
...
\end{equation}
The transport plan $\mathbf{T}$ is used to enhance the sparse $k$-NN graph $\mathbf{W}$ via:
\begin{equation}
W'_{ij} = W_{ij} \cdot \underbrace{\left(\frac{1 + (1 - |\hat{p}_i - \hat{p}_j|)}{2}\right)}_{\text{Density Correction}} + \underbrace{\gamma_{\text{OT}} \cdot [\mathbf{t}_i^\top \mathbf{t}_j - \tau]_+}_{\text{OT Bonus}}
\end{equation}
where $[\cdot]_+ = \max(0, \cdot)$ and $\mathbf{t}_i$ is the normalized transport profile.
...
\begin{algorithm}[tb]
   ...
   \label{alg:spock-opt}
\begin{algorithmic}
   \STATE {\bfseries Input:} $\{\mathbf{X}^{(v)}\}_{v=1}^V$, $K$, $k$, $M$, $\alpha$, $\varepsilon$, $T_{\max}$.
   ...
   \FOR{each view $v = 1, \ldots, V$}
       ...
       \STATE $q^{(v)} \leftarrow \mu_1/(1 + \text{CV}(\hat{p}^{(v)})) + \mu_2/(1 + \text{mean}(\bar{d}^{(v)}))$
   \ENDFOR
   ...
   \FOR{each edge $(i, j) \in \mathbf{W}$}
       ...
       \STATE $W'_{ij} \leftarrow W_{ij} \cdot \rho_{ij} + \gamma_{\text{OT}} [\mathbf{t}_i^\top \mathbf{t}_j - 0.5]_+$
   \ENDFOR
