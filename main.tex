...
For each view $v$, we seek a projection matrix $\mathbf{P}^{(v)} \in \mathbb{R}^{D^{(v)} \times d}$ and a sparse self-expression matrix $\mathbf{S}^{(v)} \in \mathbb{R}^{N \times N}$ that jointly minimize reconstruction error while preserving local manifold structure. The objective is:
\begin{equation}
\begin{aligned}
\min_{\mathbf{P}^{(v)}, \mathbf{S}^{(v)}} \mathcal{J}_1^{(v)} = & ~ \underbrace{\|\mathbf{X}^{(v)} - \mathbf{X}^{(v)}\mathbf{P}^{(v)}\mathbf{P}^{(v)\top}\|_F^2}_{\text{Reconstruction}} \\
& + \alpha \underbrace{\text{Tr}(\mathbf{P}^{(v)\top} \mathbf{X}^{(v)\top} \mathbf{L}^{(v)} \mathbf{X}^{(v)} \mathbf{P}^{(v)})}_{\text{Laplacian Smoothness}} \\
& + \beta \underbrace{\|\tilde{\mathbf{X}}^{(v)} - \mathbf{S}^{(v)}\tilde{\mathbf{X}}^{(v)}\|_F^2 + \gamma \|\mathbf{S}^{(v)}\|_1}_{\text{Sparse Self-Expression}}
\end{aligned}
\end{equation}
subject to $\mathbf{P}^{(v)\top}\mathbf{P}^{(v)} = \mathbf{I}$, $\text{diag}(\mathbf{S}^{(v)}) = 0$, and $\mathbf{S}^{(v)}$ is $k$-sparse (non-zero only for $k$-NN).
...
\textbf{Phase 1 Optimization: Block Coordinate Descent with Randomized SVD.}

The joint optimization over $(\mathbf{P}^{(v)}, \mathbf{S}^{(v)})$ is non-convex. We employ block coordinate descent (BCD) with the following alternating updates:

\textit{Step 1a: Update $\mathbf{P}^{(v)}$ via Randomized SVD.}
Fixing $\mathbf{S}^{(v)}$, the projection matrix is initialized by the top-$d$ right singular vectors of $\mathbf{X}^{(v)}$. For high-dimensional data ($D > 500$), we use randomized SVD:
\begin{enumerate}
...
\textit{Step 1c: Update $\mathbf{S}^{(v)}$ via Localized ISTA.}
Fixing $\mathbf{P}^{(v)}$, let $\tilde{\mathbf{X}}^{(v)} = \mathbf{X}^{(v)}\mathbf{P}^{(v)}$. For each point $i$, we solve:
\[
\min_{\mathbf{w}_i} \frac{1}{2}\|\tilde{\mathbf{x}}_i - \tilde{\mathbf{X}}^{(v)}_{\mathcal{N}_k(i)} \mathbf{w}_i\|_2^2 + \gamma \|\mathbf{w}_i\|_1
\]
using the Iterative Soft Thresholding Algorithm (ISTA):
\[
\mathbf{w}_i^{(t+1)} = \mathcal{S}_{\gamma/L}\left(\mathbf{w}_i^{(t)} - \frac{1}{L}\tilde{\mathbf{X}}^{(v)\top}_{\mathcal{N}_k(i)}(\tilde{\mathbf{X}}^{(v)}_{\mathcal{N}_k(i)}\mathbf{w}_i^{(t)} - \tilde{\mathbf{x}}_i)\right)
\]
where $\mathcal{S}_\tau(x) = \text{sign}(x) \max(|x| - \tau, 0)$ is the soft-thresholding operator and $L$ is the Lipschitz constant (spectral norm of local Gram matrix). Each subproblem costs $O(k^3)$, yielding $O(Nk^3)$ total.
...
\begin{algorithm}[tb]
   ...
   \label{alg:spock-opt}
\begin{algorithmic}
   \STATE {\bfseries Input:} $\{\mathbf{X}^{(v)}\}_{v=1}^V$, $K$, $k$, $M$, $\alpha$, $\beta$, $\gamma$, $\varepsilon$, $T_{\max}$.
   \STATE {\bfseries Output:} Cluster assignments $\{y_i\}_{i=1}^N$.
   \STATE
   \STATE \textcolor{blue}{\textbf{// Phase 1: Block Coordinate Descent}}
   \FOR{each view $v = 1, \ldots, V$}
       ...
       \FOR{each point $i = 1, \ldots, N$}
           \STATE $\mathbf{w}_i \leftarrow$ ISTA$(\tilde{\mathbf{x}}_i, \tilde{\mathbf{X}}^{(v)}_{\mathcal{N}_k(i)}, \gamma)$ \hfill \textit{// $O(k^3)$}
       \ENDFOR
       \STATE $\mathbf{S}^{(v)} \leftarrow$ AssembleSparse$(\{\mathbf{w}_i\})$
   \ENDFOR
   \STATE
